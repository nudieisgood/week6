  ...
  <div class="container my-5">
    ...
    <div class="p-2 p-lg-3 col-md-6">
      <div>
        ...
        <div class="form-floating mb-4">
          <VField type="text" id="origin_price" placeholder="請輸入價格" name="origin_price" class="form-control" :class="{ 'is-invalid': errors['origin_price'] }" rules="required" v-model.number="item.origin_price" />
            <ErrorMessage name="origin_price" class="invalid-feedback"/>
            <label for="unit">價格*</label>
        </div>
        <div class="form-floating mb-4">
          <VField type="text" id="price" placeholder="請輸入售價" name="price" class="form-control" :class="{ 'is-invalid': errors['price'] }" rules="required" v-model.number="item.price" />
...
export default {
  ...
  methods: {
    ...
    async addItem () {
      ...
      try {
        this.isLoading = true
        const res = await axios.put(`${import.meta.env.VITE_API}/api/${import.meta.env.VITE_PATH}/admin/product/${this.item.id}`, { data: this.item })
  ...
  async mounted () {
    ...
    this.isLoading = true
    try {
      const res = await axios.get(`${import.meta.env.VITE_API}/api/${import.meta.env.VITE_PATH}/admin/products`)
      const itemById = res.data.products.find(item => item.id === id)
      this.item = itemById
    } catch (err) {
      alert(err)
    }
